import { Link } from 'react-router-dom'

export default function Footer() {
  return (
    <footer>
      <div className="wrap">
        <div className="foot-top">
          <div className="foot-brand">
            <a className="logo" href="/" style={{ color: '#fff' }}>
              <span className="logo-circle">
                <svg style={{ width: 42, height: 42 }}><use href="#logo-nav" /></svg>
              </span>
              <span className="logo-word">
                <span className="logo-alke">Alke</span><span className="logo-buleum">buleum</span>
              </span>
            </a>
            <p>The blockchain infrastructure for digital Africa. Built for governments, institutions, the continent, and the diaspora.</p>
            <p style={{ marginTop: 8, fontSize: 13, opacity: 0.6 }}><a href="mailto:info@alkebuleum.org" style={{ color: 'inherit' }}>info@alkebuleum.org</a></p>
            <div className="foot-social">
              <a href="https://twitter.com/alkebuleum" target="_blank" rel="noopener noreferrer" aria-label="X">
                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </a>
              <a href="https://facebook.com/alkebuleum" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a href="https://instagram.com/alkebuleum" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
              </a>
              <a href="https://t.me/alkebuleum" target="_blank" rel="noopener noreferrer" aria-label="Telegram">
                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
              </a>
              <a href="https://github.com/alkebuleum" target="_blank" rel="noopener noreferrer" aria-label="GitHub">
                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.4 3-.405 1.02.005 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg>
              </a>
            </div>
          </div>
          <div className="foot-col">
            <h5>Network</h5>
            <ul>
              <li><a href="/">Overview</a></li>
              <li><a href="/#flagship">Services</a></li>
              <li><Link to="/tokenomics">Tokenomics</Link></li>
              <li><a href="https://explorer.alkebuleum.com" target="_blank" rel="noopener noreferrer">Explorer</a></li>
              <li><a href="https://explorer.alkebuleum.com" target="_blank" rel="noopener noreferrer">Status</a></li>
            </ul>
          </div>
          <div className="foot-col">
            <h5>Build</h5>
            <ul>
              <li><a href="https://docs.alkebuleum.com" target="_blank" rel="noopener noreferrer">Documentation</a></li>
              <li><a href="https://github.com/alkebuleum" target="_blank" rel="noopener noreferrer">SDKs</a></li>
              <li><a href="https://github.com/alkebuleum" target="_blank" rel="noopener noreferrer">Smart contracts</a></li>
              <li><Link to="/grants">Grants</Link></li>
              <li><a href="https://github.com/alkebuleum" target="_blank" rel="noopener noreferrer">GitHub</a></li>
            </ul>
          </div>
          <div className="foot-col">
            <h5>Ecosystem</h5>
            <ul>
              <li><Link to="/apps">Apps</Link></li>
              <li><a href="https://amvault.net" target="_blank" rel="noopener noreferrer">Amvault</a></li>
              <li><a href="https://nuruai.org" target="_blank" rel="noopener noreferrer">Nuru AI</a></li>
              <li><a href="https://jollofswap.com" target="_blank" rel="noopener noreferrer">JollofSwap</a></li>
              <li><a href="https://ugov.cc" target="_blank" rel="noopener noreferrer">Governance</a></li>
            </ul>
          </div>
          <div className="foot-col">
            <h5>Community</h5>
            <ul>
              <li><a href="/#governance">Council</a></li>
              <li><Link to="/events">Events</Link></li>
              <li><a href="https://t.me/alkebuleum" target="_blank" rel="noopener noreferrer">Telegram</a></li>
              <li><a href="/#blog">Blog</a></li>
              <li><Link to="/assets">Brand</Link></li>
            </ul>
          </div>
        </div>
        <div className="foot-bottom">
          <span>© {new Date().getFullYear()} Alkebuleum Foundation · A Pan-African Public Ledger</span>
          <div className="foot-links">
            <Link to="/privacy">Privacy</Link>
            <Link to="/terms">Terms</Link>
            <Link to="/assets">Brand</Link>
          </div>
        </div>
      </div>
    </footer>
  )
}
